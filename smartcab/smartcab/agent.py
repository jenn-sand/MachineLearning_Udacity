...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        maxV = max(v for _, v in self.Q[state].iteritems())
        maxK = []
        # Loop through the actions in this state, returning the actions that have the highest Q-value for this state
        for k, v in self.Q[state].iteritems():
            if v == maxV:
                maxK.append(k)
        # maxQ includes the max value and a random item with that associated value
        maxQ = (random.choice(maxK), maxV)
    ...
    def learn(self, state, action, reward):
        ...
        #  Ex of update rule: https://www.cs.rutgers.edu/~mlittman/courses/cps271/lect-16/node16.html
        if self.learning:
            max_future_Q = 0 #currently not calculating the maximum future state
            self.Q[state][action] = ((1 - self.alpha) * (self.Q[state][action])) + (self.alpha * (reward + max_future_Q))
